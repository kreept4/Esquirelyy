  ...
  return (
    <>
      ...
      <main className="min-h-screen" style={{ paddingTop: '64px' }}>
        ...
        <div style={{ backgroundColor: '#0F0F0F', borderTop: '2px solid #8B3A3A' }}>
          <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '4.5rem 2rem 0' }}>
            <p className="font-sans font-semibold uppercase" style={{ fontSize: '0.62rem', color: '#8B3A3A', letterSpacing: '0.18em', marginBottom: '1.25rem' }}>
              AI Career Tools
            </p>
            <h1 className="font-serif font-bold" style={{ fontSize: 'clamp(2.6rem, 5vw, 4rem)', color: '#FAF6F0', lineHeight: 1.05, letterSpacing: '-0.02em', marginBottom: '1.25rem', maxWidth: '600px' }}>
              Interview Preparation.
            </h1>
            <p className="font-sans" style={{ fontSize: '1rem', color: 'rgba(250,246,240,0.42)', lineHeight: 1.75, maxWidth: '480px', marginBottom: '3rem' }}>
          ...
          <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 2rem' }}>
            <div style={{ display: 'flex', borderBottom: '1px solid rgba(250,246,240,0.08)' }}>
              {(['generate', 'history'] as const).map(t => (
                <button
                  ...
                  style={{
                    ...
                    background: 'none',
                    cursor: 'pointer',
                    color: tab === t ? '#FAF6F0' : 'rgba(250,246,240,0.3)',
        ...
        <div style={{ backgroundColor: '#FAF6F0', minHeight: '70vh' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: '4rem', alignItems: 'start' }}>
                ...
                <div>
                  <div style={{ marginBottom: '2rem' }}>
                    <p className="font-sans font-semibold uppercase" style={{ fontSize: '0.62rem', color: '#4A4A4A', letterSpacing: '0.14em', marginBottom: '0.75rem' }}>
                      Input Mode
                    </p>
                      ...
                      {(['role', 'cv'] as const).map(m => (
                        <button
                          ...
                          style={{
                            ...
                            border: 'none',
                            cursor: 'pointer',
                            backgroundColor: mode === m ? '#0F0F0F' : '#fff',
                  ...
                  {mode === 'role' ? (
                    ...
                    <div style={{ marginBottom: '2rem' }}>
                      ...
                      <div
                        ...
                        style={{ border: '1px dashed #C8B8A8', borderRadius: '3px', padding: '2rem 1.5rem', textAlign: 'center', cursor: 'pointer', backgroundColor: cvFile ? '#F0F7EC' : '#fff' }}
                      >
                        ...
                        {!cvFile && <p className="font-sans" style={{ fontSize: '0.72rem', color: '#A0A0A0', marginTop: '0.25rem' }}>Max 10MB</p>}
                        {cvFile && (
                          <button onClick={e => { e.stopPropagation(); setCvFile(null) }} className="font-sans" style={{ fontSize: '0.72rem', color: '#8B3A3A', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>
                            Remove
                          </button>
                        )}
                      </div>
                      ...
                      {PERSONAS.map(p => (
                        <button
                          ...
                          style={{
                            ...
                            gap: '0.9rem',
                            padding: '0.8rem 1rem',
                            border: persona === p.id ? '1px solid #0F0F0F' : '1px solid #E8E0D5',
                            borderRadius: '3px',
                            background: persona === p.id ? '#0F0F0F' : '#fff',
                            cursor: 'pointer',
                            textAlign: 'left',
                            transition: 'all 0.15s',
                          }}
                        >
                          ...
                          <div>
                            ...
                            <p className="font-sans" style={{ fontSize: '0.72rem', color: persona === p.id ? 'rgba(250,246,240,0.45)' : '#6A6A6A', marginTop: '1px' }}>{p.desc}</p>
                          </div>
                        </button>
                      ))}
                    </div>
                  ...
                  {error && <p className="font-sans" style={{ fontSize: '0.78rem', color: '#B5451B', marginBottom: '1rem' }}>{error}</p>}
                  ...
                  <button
                    ...
                    style={{
                      width: '100%',
                      padding: '1rem',
                      backgroundColor: loading ? '#4A4A4A' : '#0F0F0F',
                      color: '#FAF6F0',
                      border: 'none',
                      borderRadius: '3px',
                      fontSize: '0.75rem',
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                        {result.questions.map((q, i) => (
                          <div key={q.id || i} style={{ border: '1px solid #E8E0D5', borderRadius: '3px', overflow: 'hidden', backgroundColor: '#fff' }}>
                            <button
                            ...
                            >
                              ...
                              <div style={{ flex: 1 }}>
                                <p className="font-sans" style={{ fontSize: '0.9rem', color: '#1A1A1A', lineHeight: 1.65, marginBottom: '0.5rem' }}>{q.question}</p>
                                <span className="font-sans font-semibold uppercase" style={{ fontSize: '0.58rem', letterSpacing: '0.1em', color: categoryColor(q.category), backgroundColor: categoryColor(q.category) + '12', padding: '2px 7px', borderRadius: '2px' }}>
                                  {q.category}
                                </span>
                              </div>
                              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#C8B8A8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0, marginTop: '3px', transform: expandedQ === q.id ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>
                            ...
                            {expandedQ === q.id && (
                              <div style={{ padding: '0 1.25rem 1.1rem 3.25rem', borderTop: '1px solid #F0EBE3' }}>
                                ...
                                <p className="font-sans" style={{ fontSize: '0.85rem', color: '#4A4A4A', lineHeight: 1.7 }}>{q.whyTheyAsk}</p>
                              </div>
                            )}
                          </div>
                        ))}
              ...
              <div style={{ maxWidth: '700px' }}>
                {history.length === 0 ? (
                  <div style={{ textAlign: 'center', padding: '5rem 2rem' }}>
                    <p className="font-sans" style={{ fontSize: '0.9rem', color: '#A0A0A0' }}>No past sessions yet. Generate your first set of questions.</p>
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {history.map(s => (
                          ...
                          {s.questions.map((q: any, i: number) => (
                            <div key={q.id || i} style={{ display: 'flex', gap: '1rem' }}>
                              ...
                              <div>
                                ...
                                {q.category && <span className="font-sans font-semibold uppercase" style={{ fontSize: '0.58rem', letterSpacing: '0.1em', color: categoryColor(q.category) }}>{q.category}</span>}
                              </div>
                            </div>
                          ))}
                        </div>
