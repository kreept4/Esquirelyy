/**
 * Send the roles announcement to one address, on demand.
 *
 * Sibling of send-test-welcome.mjs, and it exists for a reason that only became
 * obvious the first time somebody tried to test a broadcast: send-new-roles.mjs
 ...
 * send-new-roles.mjs reads `profiles.full_name`, so this reads
 ...
 *
 * Usage:  node scripts/send-test-new-roles.mjs you@example.com
 */
...
if (!to) {
  console.error('usage: node scripts/send-test-new-roles.mjs you@example.com')
  process.exit(1)
}
if (process.argv[3]) {
...
/* Bundled with esbuild rather than imported through the ts-resolve hook the
   welcome test uses. This template imports lib/new-roles through the '@/' alias
   and that hook does not resolve it; send-new-roles.mjs already solved this the
...
const bundle = join(tmpdir(), `esq-test-new-roles-${process.pid}.mjs`)
execSync(
...
const { subject, html, text } = newRolesEmail({ name, siteUrl })
...
const res = await fetch('https://api.brevo.com/v3/smtp/email', {
  ...
  body: JSON.stringify({
    ...
    to: [{ email: to, name: name || undefined }], // same shape send-new-roles.mjs uses
    subject,
    htmlContent: html,
    textContent: text,
  }),
