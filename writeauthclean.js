...
const rawSvg = fs.readFileSync('C:\\Users\\Barr. Tobi Ogunleye\\Downloads\\Curious-amico.svg', 'utf8')
  ...
  .replace(/<svg /, '<svg style="width:100%;height:auto;max-height:220px;display:block;" ');

const svgEscaped = rawSvg.replace(/`/g, '\\`').replace(/\$/g, '\\$');
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#FAF6F0', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1.5rem' }}>
      <div style={{ width: '100%', maxWidth: '400px' }}>
        ...
        <div style={{ marginTop: '2rem', marginBottom: '2rem', textAlign: 'center' }}>
          <h1 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '1.75rem', fontWeight: 700, color: '#1A1A1A', marginBottom: '0.4rem', lineHeight: 1.2 }}>Welcome back.</h1>
          <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.875rem', color: '#4A4A4A', lineHeight: 1.6 }}>Sign in to continue to Esquirely.</p>
        </div>

        <button onClick={handleGoogleLogin} className="auth-btn-google" style={{ marginBottom: '1.25rem' }}>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              <input type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required placeholder="Your password" className="auth-input" style={{ paddingRight: '2.75rem' }} />
