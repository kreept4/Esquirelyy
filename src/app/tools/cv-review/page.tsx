          ...
          <>
            ...
            <div style={{ maxWidth: '640px', margin: '0 auto', padding: '0 2rem 6rem' }}>
              ...
              <div style={{ marginBottom: '2rem' }}>
                ...
                <select value={careerStage} onChange={e => setCareerStage(e.target.value)} style={{ width: '100%', padding: '0.6rem 0', fontFamily: 'DM Sans, sans-serif', fontSize: '0.92rem', color: ink, backgroundColor: 'transparent', border: 'none', borderBottom: '1px solid ' + rule, outline: 'none', cursor: 'pointer' }}>
                ...
                {file ? (
                  <div>
                    ...
                    <button
                      ...
                      style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.76rem', color: muted, background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}
                    >
              ...
              {error && (
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', padding: '0.5rem 0', marginBottom: '1.5rem' }}>
                  ...
                  <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.82rem', color: '#B5451B' }}>{error}</p>
                </div>
              )}
        ...
        {result && (
          <div style={{ maxWidth: '700px', margin: '0 auto', padding: '5rem 2rem 6rem' }}>
            ...
            <div style={{ borderTop: '1px solid ' + rule, paddingTop: '2.5rem', marginBottom: '3.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'stretch' }}>
                {Object.entries(result.scores).map(([key, value], i) => (
                  ...
                  >
                    <p style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: 'clamp(2.2rem, 5vw, 3.2rem)', fontWeight: 700, color: accent, lineHeight: 1, marginBottom: '0.5rem' }}>
                      {value}
                    </p>
                    <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase' as const, color: muted }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '3rem', marginBottom: '3.5rem' }}>
              <div>
                <h2 style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: sage, marginBottom: '1.25rem' }}>
                  What's Working
                </h2>
                <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '1rem' }}>
                  {result.strengths.map((s, i) => (
                    <p key={i} style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.86rem', color: ink, lineHeight: 1.65, paddingLeft: '0.9rem', borderLeft: '2px solid ' + sage }}>
                      {s}
                    </p>
                  ))}
                </div>
              ...
              <div>
                <h2 style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: accent, marginBottom: '1.25rem' }}>
                  Where to Improve
                </h2>
                <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '1rem' }}>
                  {result.weaknesses.map((w, i) => (
                    <p key={i} style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.86rem', color: ink, lineHeight: 1.65, paddingLeft: '0.9rem', borderLeft: '2px solid ' + accent }}>
                      {w}
                    </p>
                  ))}
                </div>
            ...
            {result.rewrites && result.rewrites.length > 0 && (
              <div style={{ marginBottom: '3.5rem' }}>
                <h2 style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: muted, marginBottom: '2rem' }}>
                  Suggested Rewrites
                </h2>
                <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '2.5rem' }}>
                  {result.rewrites.map((r, i) => (
                    <div key={i} style={{ borderTop: i > 0 ? '1px solid ' + rule : 'none', paddingTop: i > 0 ? '2.5rem' : 0 }}>
                      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: muted, marginBottom: '0.5rem' }}>Before</p>
                      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.88rem', color: muted, lineHeight: 1.7, fontStyle: 'italic', marginBottom: '1.25rem' }}>{r.original}</p>
                      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: accent, marginBottom: '0.5rem' }}>After</p>
                      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.88rem', color: ink, lineHeight: 1.7, fontWeight: 500, marginBottom: '1rem' }}>{r.improved}</p>
                      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.8rem', color: muted, lineHeight: 1.65 }}>{r.why}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <p style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: 'clamp(1.3rem, 3vw, 1.7rem)', fontWeight: 500, color: ink, lineHeight: 1.55, fontStyle: 'italic', borderTop: '1px solid ' + rule, paddingTop: '2.5rem', marginBottom: '2.5rem' }}>
              {result.closingNote}
            </p>
          ...
          <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(26,26,26,0.4)', zIndex: 200, display: 'flex', justifyContent: 'flex-end' }} onClick={() => setShowHistory(false)}>
            <div style={{ width: '100%', maxWidth: '420px', height: '100%', backgroundColor: cream, overflowY: 'auto' as const, padding: '2rem' }} onClick={e => e.stopPropagation()}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <h3 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '1.3rem', fontWeight: 700, color: ink }}>Review History</h3>
              ...
              {!historyLoading && history.length === 0 && (
                <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.85rem', color: muted }}>No reviews yet. Your past CV reviews will appear here.</p>
              )}
              ...
              <div style={{ display: 'flex', flexDirection: 'column' as const, gap: '0.75rem' }}>
                {history.map(item => (
                  <button key={item.id} onClick={() => loadFromHistory(item)} style={{ textAlign: 'left' as const, padding: '1rem', backgroundColor: '#fff', border: '0.5px solid ' + rule, cursor: 'pointer' }}>
                    <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.85rem', fontWeight: 600, color: ink, marginBottom: '0.25rem' }}>{item.file_name || 'CV Review'}</p>
