...
/**
 * The directory's interactive half: search, the filter selects, the layout
 * switch and the card grid.
 *
 * This is everything that used to be `firms/page.tsx`, moved here unchanged so
 * that the route file can be a server component. It had to move, because a
 * `'use client'` page cannot export `metadata`, and so the most valuable page on
 * the site — a directory of sixty seven Nigerian law firms — was shipping the
 * generic sitewide title, the generic sitewide description and, until the same
 * commit, a canonical pointing at the homepage. Nothing in its head said it was
 * a directory of law firms at all.
 *
 * The split is the shape /jobs and /scholarships already use. The state lives
 * here because it is state; the head lives in page.tsx because it is a fact
 * about the route.
 */
 ...
 *
 *  Both dense modes were drawn against a wide viewport and neither survives a
 *  phone: the grid's two 163px tiles crowd the mark against a name that wraps,
 *  and the list row's 4.25rem footer indent leaves the cities about 130px to
 *  sit in. The control goes with them, hidden at the same 640px the header
 *  panel already breaks at (see .view-toggle in globals.css).
 *
 *  Hiding alone would not be enough, which is why the view is pinned below.
 *  The choice is in localStorage, so a reader who picked the grid on a laptop
 *  would open the directory on their phone inside a layout with no visible way
 *  out of it. Pinning also covers a desktop window dragged narrow, and it
 *  leaves the stored preference untouched: widen the window and the grid comes
 *  back. */
...
export default function FirmsClient() {
  ...
  /* False on the server and on the first client frame, which is the safe way
     round: the server has no viewport to measure, and 'detailed' is what it
     renders anyway, so the first paint agrees with the markup either way. */
  ...
  /* What the page actually draws. `view` stays the reader's stored choice. */
  const activeView: View = isPhone ? 'detailed' : view
        ...
        >
            ...
            <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
              ...
              {/* Pushed to the far end of the row, because it is not a filter.
                  ...
                  cleared-filters count. Gone entirely below 640px — see
                  PHONE_QUERY above for why. */}
              ...
              {filtered.map(firm => (
                <Link key={firm.slug} href={`/firms/${firm.slug}`}>
                  ...
                  <div className="firm-card-head">
                    <FirmAvatar firm={firm} compact={activeView === 'list'} />
