    ...
    >
      ...
      <div style={{ padding: '0.9rem 1rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.5rem', marginBottom: '0.3rem' }}>
          <p style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '0.88rem', fontWeight: 600, color: '#1A1A1A', lineHeight: 1.3, flex: 1 }}>{app.role}</p>
        ...
        <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.78rem', fontWeight: 600, color: '#8B3A3A', marginBottom: '0.6rem' }}>{app.firm}</p>
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginBottom: '0.75rem' }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: '3px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.7rem', color: '#6A6A6A' }}><IconMapPin /> {app.location}</span>
          <span style={{ display: 'flex', alignItems: 'center', gap: '3px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.7rem', color: '#6A6A6A' }}><IconCalendar /> Applied {app.dateApplied}</span>
        </div>
        <span style={{ display: 'inline-block', fontFamily: 'DM Sans, sans-serif', fontSize: '0.62rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: '#8B3A3A', backgroundColor: '#EBF0F7', padding: '2px 7px', borderRadius: '2px' }}>{app.type}</span>
        {expanded && (
          <div style={{ marginTop: '0.85rem', paddingTop: '0.85rem', borderTop: '0.5px solid #E8E0D5' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
              <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.7rem', color: '#6A6A6A' }}>Deadline</span>
              <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.72rem', fontWeight: 600, color: '#1A1A1A' }}>{app.deadline}</span>
            </div>
            <textarea value={noteVal} onChange={e => setNoteVal(e.target.value)} onBlur={() => onNoteChange(app.id, noteVal)} placeholder="Add a note..." rows={2}
              style={{ width: '100%', fontFamily: 'DM Sans, sans-serif', fontSize: '0.75rem', color: '#4A4A4A', backgroundColor: '#F5F0EB', border: '0.5px solid #E8E0D5', borderRadius: '2px', padding: '0.5rem 0.6rem', resize: 'none' as const, outline: 'none', lineHeight: 1.6, marginBottom: '0.75rem', boxSizing: 'border-box' as const }} />
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'relative' as const }}>
              <div style={{ position: 'relative' as const }}>
                <button onClick={() => setShowStatusMenu(!showStatusMenu)}
                  style={{ display: 'flex', alignItems: 'center', gap: '4px', fontFamily: 'DM Sans, sans-serif', fontSize: '0.68rem', fontWeight: 600, color: col.color, background: 'none', border: '0.5px solid currentColor', borderRadius: '2px', padding: '3px 8px', cursor: 'pointer', opacity: 0.85 }}>
                  ...
                  <div style={{ position: 'absolute' as const, bottom: '110%', left: 0, backgroundColor: '#FDFAF5', border: '0.5px solid #E8E0D5', borderRadius: '3px', boxShadow: '0 8px 24px rgba(0,0,0,0.1)', zIndex: 10, minWidth: '140px', overflow: 'hidden' }}>
                    {COLUMNS.filter(c => c.status !== app.status).map(c => (
                      <button key={c.status} onClick={() => { onStatusChange(app.id, c.status); setShowStatusMenu(false) }}
                        style={{ display: 'flex', alignItems: 'center', gap: '8px', width: '100%', fontFamily: 'DM Sans, sans-serif', fontSize: '0.75rem', color: '#1A1A1A', background: 'none', border: 'none', padding: '0.6rem 0.9rem', cursor: 'pointer', textAlign: 'left' as const, borderBottom: '0.5px solid #F0EBE3' }}
...
function AddModal({ onClose, onAdd }: { onClose: () => void; onAdd: (app: Omit<Application, 'id'>) => Promise<void> }) {
  ...
  const inputStyle: React.CSSProperties = { width: '100%', fontFamily: 'DM Sans, sans-serif', fontSize: '0.85rem', color: '#1A1A1A', backgroundColor: '#F5F0EB', border: '0.5px solid #E8E0D5', borderRadius: '2px', padding: '0.6rem 0.75rem', outline: 'none', boxSizing: 'border-box' }
  const labelStyle: React.CSSProperties = { display: 'block', fontFamily: 'DM Sans, sans-serif', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#4A4A4A', marginBottom: '0.35rem' }
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 200, backgroundColor: 'rgba(139,58,58,0.45)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}
      ...
      <div style={{ backgroundColor: '#FAF7F2', border: '0.5px solid #E8E0D5', borderRadius: '4px', padding: '2rem', width: '100%', maxWidth: '500px', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.75rem' }}>
          <div>
            <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#8B3A3A', opacity: 0.6, marginBottom: '0.2rem' }}>Application Tracker</p>
            <h2 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '1.4rem', fontWeight: 700, color: '#1A1A1A' }}>Log an application</h2>
          </div>
        ...
        <button onClick={submit} disabled={saving}
          style={{ marginTop: '1.5rem', width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', backgroundColor: saving ? '#A0A0A0' : '#8B3A3A', color: '#FAF7F2', fontFamily: 'DM Sans, sans-serif', fontSize: '0.78rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', border: 'none', borderRadius: '2px', padding: '0.85rem', cursor: saving ? 'not-allowed' : 'pointer' }}>
...
export default function TrackerPage() {
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', backgroundColor: '#FAF7F2', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.85rem', color: '#6A6A6A' }}>Loading your applications...</p>
    </div>
  )
      ...
      <main style={{ backgroundColor: '#FAF7F2', minHeight: '100vh', paddingTop: '64px' }}>
        <div style={{ borderBottom: '0.5px solid #E8E0D5', backgroundColor: '#FAF7F2', padding: '3rem 2rem 0', maxWidth: '1440px', margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: '1.5rem', paddingBottom: '2rem' }}>
            <div>
              <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: '#8B3A3A', opacity: 0.6, marginBottom: '0.4rem' }}>Application Tracker</p>
              <h1 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: 'clamp(1.8rem, 3vw, 2.6rem)', fontWeight: 800, color: '#1A1A1A', lineHeight: 1.1, marginBottom: '0.5rem' }}>Where you stand.</h1>
              <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.88rem', color: '#6A6A6A', lineHeight: 1.6 }}>
                ...
                <code style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.8rem', backgroundColor: '#EBF0F7', padding: '1px 5px', borderRadius: '2px', color: '#8B3A3A' }}>you@mail.esquirely.app</code>
                {' '}and we log them automatically.
              </p>
            </div>
            <button onClick={() => setShowModal(true)}
              style={{ display: 'flex', alignItems: 'center', gap: '8px', backgroundColor: '#8B3A3A', color: '#FAF7F2', fontFamily: 'DM Sans, sans-serif', fontSize: '0.75rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', border: 'none', borderRadius: '2px', padding: '0.75rem 1.5rem', cursor: 'pointer', flexShrink: 0 }}
          ...
          <div style={{ display: 'flex', borderTop: '0.5px solid #E8E0D5', flexWrap: 'wrap' }}>
            {STATS.map(({ label, value }, i) => (
              <div key={label} style={{ flex: '1 1 120px', padding: '1.25rem 1.75rem 1.25rem 0', borderRight: i < STATS.length - 1 ? '0.5px solid #E8E0D5' : 'none', marginRight: i < STATS.length - 1 ? '1.75rem' : 0 }}>
                <p style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '1.75rem', fontWeight: 700, color: '#1A1A1A', lineHeight: 1, marginBottom: '0.2rem' }}>{value}</p>
                <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.65rem', fontWeight: 600, letterSpacing: '0.1em', textTransform: 'uppercase', color: '#6A6A6A' }}>{label}</p>
              </div>
            ))}
          </div>
        </div>
                ...
                <div key={col.status} style={{ flex: '1 1 200px', minWidth: '200px', maxWidth: '280px', backgroundColor: '#F0EBE3', border: '0.5px solid #E8E0D5', borderRadius: '3px', overflow: 'hidden' }}>
                  <div style={{ padding: '0.85rem 1rem', borderBottom: '0.5px solid #E8E0D5', backgroundColor: '#EAE4DB', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                      ...
                      <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.09em', textTransform: 'uppercase', color: col.color }}>{col.status}</span>
                    </div>
                    <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.68rem', fontWeight: 700, color: '#A0A0A0' }}>{colApps.length}</span>
                  </div>
                  <div style={{ padding: '0.75rem 0.6rem', minHeight: '120px' }}>
                    {colApps.length === 0 ? (
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem 1rem', color: '#C0B8AE', gap: '0.5rem' }}>
                        <IconInbox size={28} />
                        <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.7rem', textAlign: 'center', lineHeight: 1.5 }}>Nothing here yet</p>
                      </div>
                    ) : (
                  ...
                  <button onClick={() => setShowModal(true)}
                    style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px', padding: '0.65rem', background: 'none', border: 'none', borderTop: '0.5px solid #E8E0D5', cursor: 'pointer', fontFamily: 'DM Sans, sans-serif', fontSize: '0.68rem', color: '#A0A0A0', transition: 'background-color 0.15s ease, color 0.15s ease' }}
                    onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#EAE4DB'; e.currentTarget.style.color = '#8B3A3A' }}
        ...
        <div style={{ maxWidth: '1440px', margin: '0 auto 4rem', padding: '0 2rem' }}>
          <div style={{ backgroundColor: '#EBF0F7', border: '0.5px solid #C8D8EC', borderRadius: '3px', padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            <div style={{ color: '#8B3A3A', flexShrink: 0 }}><IconSend size={18} /></div>
            <div style={{ flex: 1 }}>
              <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.82rem', fontWeight: 600, color: '#8B3A3A', marginBottom: '0.2rem' }}>Auto-log with email forwarding</p>
              <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.78rem', color: '#4A6A8A', lineHeight: 1.5 }}>Forward any application confirmation to your Esquirely address and we parse the firm, role, and status automatically.</p>
            </div>
            <button style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.7rem', fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#8B3A3A', backgroundColor: 'transparent', border: '0.5px solid #8B3A3A', borderRadius: '2px', padding: '0.5rem 1rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px', flexShrink: 0, opacity: 0.8 }}>
