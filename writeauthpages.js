...
const rawSvg = fs.readFileSync('C:\\Users\\Barr. Tobi Ogunleye\\Downloads\\Curious-amico.svg', 'utf8')
  ...
  .replace(/<svg /, '<svg style="width:100%;height:auto;max-height:340px;display:block;" ');

// Escape backticks for template literal
const svgEscaped = rawSvg.replace(/`/g, '\\`');
...
export default function AuthIllustration() {
  return (
    <div
      ...
      dangerouslySetInnerHTML={{ __html: \`${svgEscaped}\` }}
    />
  )
}
`);
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', backgroundColor: '#FAF6F0' }}>
      <div style={{ flex: 1, backgroundColor: '#8B3A3A', padding: '3rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }} className="auth-left-panel">
        ...
        <div>
          <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.62rem', fontWeight: 600, letterSpacing: '0.18em', textTransform: 'uppercase', color: 'rgba(250,246,240,0.4)', marginBottom: '1rem' }}>Why Esquirely?</p>
          <blockquote style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: 'clamp(0.9rem, 1.4vw, 1.15rem)', fontStyle: 'italic', color: '#FAF6F0', lineHeight: 1.8, margin: 0 }}>
            "Every Nigerian law student deserves to know about every opportunity available to them. Not just the ones with the right connections. That is what we are here to fix."
          </blockquote>
          <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.73rem', fontWeight: 600, color: 'rgba(250,246,240,0.45)', marginTop: '1rem' }}>Ogunleye Boluwatife & Ogunleye Ipinuoluwa, Co-founders</p>
        </div>
      </div>
        ...
        <div style={{ width: '100%', maxWidth: '380px' }}>
          {success ? (
            <div style={{ textAlign: 'center' }}>
              <div style={{ width: '56px', height: '56px', borderRadius: '50%', backgroundColor: '#F2EBE1', border: '0.5px solid #E8E0D5', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1.5rem' }}>
            ...
            <>
              <div style={{ marginBottom: '2.5rem' }}>
                <h1 style={{ fontFamily: 'Playfair Display, Georgia, serif', fontSize: '2rem', fontWeight: 700, color: '#1A1A1A', marginBottom: '0.4rem', lineHeight: 1.15 }}>Create your account.</h1>
                <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.88rem', color: '#4A4A4A', lineHeight: 1.6 }}>Join thousands of Nigerian legal professionals on Esquirely.</p>
              </div>

              <button onClick={handleGoogleSignup} style={{ width: '100%', padding: '0.8rem', backgroundColor: '#fff', border: '0.5px solid #E8E0D5', borderRadius: '2px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', fontFamily: 'DM Sans, sans-serif', fontSize: '0.85rem', fontWeight: 500, color: '#1A1A1A', cursor: 'pointer', marginBottom: '1.5rem', transition: 'border-color 0.2s ease' }}
                onMouseEnter={e => e.currentTarget.style.borderColor = '#8B3A3A'}
                onMouseLeave={e => e.currentTarget.style.borderColor = '#E8E0D5'}
              >
                <svg width="18" height="18" viewBox="0 0 24 24">
