    ...
    try {
      ...
      if (user) {
        await supabase.from('interview_sessions').insert({
          user_id: user.id,
          target_role: mode === 'role' ? targetRole : null,
          persona,
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'2.5rem' }}>
              ...
              <div>
                ...
                {mode === 'role' ? (
                  <>
                    <div style={{ marginBottom:'1rem' }}>
                      <label style={{ display:'block', fontFamily:'DM Sans, sans-serif', fontSize:'0.65rem', fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:'#4A4A4A', marginBottom:'0.4rem' }}>
                        Target Role *
                      </label>
                      <input value={targetRole} onChange={e => setTargetRole(e.target.value)}
                        placeholder="e.g. Associate, Banking & Finance" style={inp} />
                    </div>
                    <div style={{ marginBottom:'1.5rem' }}>
                      <label style={{ display:'block', fontFamily:'DM Sans, sans-serif', fontSize:'0.65rem', fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:'#4A4A4A', marginBottom:'0.4rem' }}>
                        Practice Area
                  ...
                  <div style={{ marginBottom:'1.5rem' }}>
                    ...
                    <div
                      ...
                      style={{
                        border:'0.5px dashed #C8B8A8', borderRadius:'2px',
                        padding:'1.5rem', textAlign:'center', cursor:'pointer',
                        backgroundColor: cvFile ? '#F0F7EC' : '#FAF7F2',
                    ...
                    >
                      ...
                      {cvFile && (
                        <button onClick={e => { e.stopPropagation(); setCvFile(null) }}
                          style={{ marginTop:'0.5rem', fontFamily:'DM Sans, sans-serif', fontSize:'0.7rem', color:'#8B3A3A', background:'none', border:'none', cursor:'pointer', textDecoration:'underline' }}>
                          Remove
                        </button>
                      )}
                    </div>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:'0.5rem' }}>
                    {PERSONAS.map(p => (
                      <button key={p.id} onClick={() => setPersona(p.id)} style={{
                        ...
                        border: persona === p.id ? '0.5px solid #8B3A3A' : '0.5px solid #E8E0D5',
                        borderRadius:'2px', background: persona === p.id ? '#FDF5F5' : '#fff',
                ...
                <button onClick={handleGenerate} disabled={loading} style={{
                  ...
                  color:'#FAF7F2', border:'none', borderRadius:'2px',
              ...
              <div>
                {questions.length === 0 ? (
                  <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100%', minHeight:'300px', color:'#C0B8AE', textAlign:'center' }}>
                    ...
                    <p style={{ fontFamily:'DM Sans, sans-serif', fontSize:'0.82rem', lineHeight:1.6 }}>
                      Configure your session and generate questions to see them here.
                    </p>
                  </div>
                ) : (
                  <div>
                    <p style={{ fontFamily:'DM Sans, sans-serif', fontSize:'0.65rem', fontWeight:700, letterSpacing:'0.1em', textTransform:'uppercase', color:'#8B3A3A', marginBottom:'1rem' }}>
                      {questions.length} Questions Generated
                    </p>
                    <div style={{ display:'flex', flexDirection:'column', gap:'0.75rem' }}>
                      {questions.map((q, i) => (
                        <div key={i} style={{ backgroundColor:'#fff', border:'0.5px solid #E8E0D5', borderRadius:'2px', padding:'1rem 1.1rem' }}>
                          <p style={{ fontFamily:'DM Sans, sans-serif', fontSize:'0.65rem', fontWeight:700, color:'#8B3A3A', marginBottom:'0.3rem' }}>Q{i+1}</p>
                          <p style={{ fontFamily:'DM Sans, sans-serif', fontSize:'0.85rem', color:'#1A1A1A', lineHeight:1.65 }}>{q}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
                  {history.map(s => (
                    <details key={s.id} style={{ backgroundColor:'#fff', border:'0.5px solid #E8E0D5', borderRadius:'2px', overflow:'hidden' }}>
                      <summary style={{ padding:'1rem 1.25rem', cursor:'pointer', listStyle:'none', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                        <div>
                          ...
                          <p style={{ fontFamily:'DM Sans, sans-serif', fontSize:'0.7rem', color:'#6A6A6A', marginTop:'0.15rem' }}>
                            {PERSONAS.find(p => p.id === s.persona)?.label || s.persona} &middot; {s.createdAt}
                          </p>
                        </div>
                      ...
                      <div style={{ borderTop:'0.5px solid #E8E0D5', padding:'1rem 1.25rem', display:'flex', flexDirection:'column', gap:'0.6rem' }}>
                        {s.questions.map((q, i) => (
                          <div key={i} style={{ display:'flex', gap:'0.75rem' }}>
                            ...
                            <p style={{ fontFamily:'DM Sans, sans-serif', fontSize:'0.82rem', color:'#1A1A1A', lineHeight:1.6 }}>{q}</p>
                          </div>
                        ))}
                      </div>
                    </details>
